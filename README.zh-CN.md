...
[这个点子怎么来的](#从编辑器烟花到智能体反馈) · [动态预览](#动态预览) · [快速安装](#快速安装) · [视觉系统](#不是加载转圈而是一套视觉语言) · [隐私](#本地运行隐私优先) · [文档](#文档)
...
这个点子来自以前 VS Code 里的 Power Mode：敲一下键，编辑器就用粒子、震动和 Combo 回你一下。它有点多余，却也正因为这种恰到好处的多余，让写代码突然有了手感。真正让人上头的并不只是爆炸，而是“我做了什么，编辑器马上有回应”这一小段反馈回路。

最早的 Codex 原型也几乎照搬了这个思路：一次大 patch 会变成一阵高速虚拟输入，修改冒火花，测试给能量充电，验证通过再放一场胜利动画。画面是热闹了，但总觉得哪里不对。Codex 并不是坐在键盘前替你打字；它真正有意思的过程，是理解需求、查看项目、决定怎么改、调用工具、检查结果、等待授权，以及出错后的恢复。

于是问题变成了：**如果 Power Mode 响应的不再只是按键，而是工作本身，会是什么样？**

这就是现在的 Codex Power Mode。**经典 Power Mode** 仍然保留老式打字反馈，但主 HUD 跟随的是 Codex 的真实生命周期：观察时能量向内汇聚，执行时机械结构向前推进，验证时四个节点依次锁定，失败后回路反转修复；有效工作积累能量，完成动画也只庆祝它真正做到的结果。它保留了旧 Power Mode 那种即时的爽感，又长出了一套更适合智能体的视觉语言。
