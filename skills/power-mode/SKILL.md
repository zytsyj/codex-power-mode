...
On macOS, settings live under the menu-bar bolt. It controls Focus/Arcade/Classic Power Mode, ten bounded Energy gain presets from 0.30× through 1.50×, independent low/normal/high effect intensity, agent Combo visibility, opt-in Typing Combo, cursor effects (Off/Sparks/Neon/Orbit/Ripple/Prism/Liquid Wormhole/Glitch Slices/Soft Tentacles/Chinese Meme Words/Hands-behind Possum/Fresh Cat/Knife-shield Dog/Elegant Person), auto-hide versus a quiet Idle orb, the immediate/2-second/6-second auto-hide delay, English/Chinese, size, reduced motion, inactive-Codex behavior, and activity source. Classic Power Mode hides the Energy orb and semantic choreography, forces Typing Combo on, and keeps only cursor feedback plus a centered `×N` counter and lifetime line; it has no idle hit target. Energy gain changes are read from the shared settings file on the next real event and need no restart; every Energy source uses the same Peak boundary at `900`. Typing Combo requires macOS Accessibility permission. Enabling it prompts for the stable Codex Power Mode app identity and opens the correct System Settings pane; the bolt menu also exposes Grant cursor access when needed, and permission polling activates the feature without dragging an executable or restarting the HUD. It listens only while Codex is the foreground app, ignores deletion/control/navigation keys, and keeps only a capped rhythm count and timestamp—never text or key values. Its large `×N` counter is separate from the orb; cursor particles are separately configurable, and submit streams the counter into the orb in Focus and Arcade while Classic clears it without displaying an orb injection. Activity source offers Focus (keep one conversation), Global (follow the latest conversation with isolated state), and Mix (share one Energy/Combo pool for all Codex app conversations). Every Mix conversation end receives a brief completion result stamp without resetting the shared pool; the final conversation receives the full Complete family. The hide delay begins only after completion feedback, Combo, and energy return have settled; it does not extend an active Combo. The inactive behavior can hide the HUD, keep it over the last Codex window position, or follow the current foreground app. Hiding Combo also disables its otherwise invisible high-frequency decay redraw. The visible orb is directly draggable in Focus and Arcade, and the active `×N` counter is directly draggable in Classic. Only the compact visible target captures the mouse, while all empty overlay space stays click-through. Releasing saves the position and restores normal interaction. Settings survive overlay restarts, and the menu reports historical best energy and Combo.

Direct dragging keeps the full HUD inside the Codex window and snaps near edges. Position controls are intentionally absent from the settings menu; the saved drag location remains available across restarts. Smart placement avoids the title bar and reserves space for common Codex side panels until a manually dragged position takes over.
