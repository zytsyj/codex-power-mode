...
- Energy spans `0–999`: Wake, Charge, Drive, Critical, and an evidence-backed verified Peak. A bright 300-degree gauge fills within each tier, warns above 90%, and resets after the new level establishes; decay drains to a boundary, morphs the orb backward, then resumes from a full lower-tier gauge. Evolution is cumulative but visually bounded: Wake awakens a breathing core, Charge adds three collector nodes, Drive adds two directional fins, Critical links the collectors into a stable lattice, and Peak synchronizes the retained systems beneath a white-gold crown. Previous modules become quiet, current-palette structure while only the newest module moves. Downgrades remove those abilities in reverse order.
