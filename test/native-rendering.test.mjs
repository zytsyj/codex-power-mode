...
test("Accessibility onboarding uses a stable app identity and activates without restarting", async () => {
  ...
  assert.match(overlay, /trusted != self\.lastPermissionGranted/);
  assert.match(overlay, /self\.stopEventMonitoring\(\)/);
  assert.match(overlay, /private var permissionRequestPending = false/);
  assert.match(overlay, /pendingAccessibilityAction = \.enableTypingCombo/);
  assert.match(overlay, /pendingAccessibilityAction = \.enterClassicMode/);
  assert.match(overlay, /private func accessibilityPermissionChanged\(granted: Bool\)/);
  assert.match(overlay, /preferences\.disableTypingComboForMissingPermission\(\)/);
  assert.match(overlay, /if \$0\.preset == "classic" \{ \$0\.preset = "focus" \}/);
  assert.doesNotMatch(overlay, /Typing Combo \(permission needed\)/);
  assert.doesNotMatch(overlay, /输入连击（等待授权）/);
  assert.doesNotMatch(overlay, /exclamationmark\.triangle\.fill/);
  ...
  assert.match(overlay, /case \.enableTypingCombo:\s+preferences\.enableTypingCombo\(\)/);
