...
The connected-mechanism Energy renderer measures 74 layers/65 animations for Focus, 76/67 for Arcade, and 67/22 for Reduce Motion at the synthetic peak. Its six reusable node layers, shared bus, propulsion ports, stabilizer, and crown remain below the enforced 96-layer/88-animation ceilings without allocating new steady-state layers as Energy changes. Reduce Motion is explicitly required to allocate fewer transient animations than Focus.
